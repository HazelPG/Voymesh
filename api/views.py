...
class MisionMinList(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        query = Mision.objects.filter(fecha_eliminacion=None).order_by('-id')
...
class LastMisionMinList(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        for key in serializer.data['mediciones']:
            data = ast.literal_eval(key['data'])
            temperatura.append(int(data['temperatura']))
            humedad.append(int(data['humedad']))
            gas.append(int((data['gas'])))
            concentracion.append(float(data['concentracion']))

        if serializer:
            data = {'message': 'ultima mision', 'code': 1, 'data': serializer.data, 'temperatura' : temperatura, 'humedad': humedad, 'gas': gas, 'concentracion':concentracion }
            return Response(data, status=200)
        else:
            data = {'message': 'ultima mision', 'code': 1, 'data': [], 'temperatura' : temperatura, 'humedad': humedad, 'gas': gas, 'concentracion':concentracion}
            return Response(data, status=200)